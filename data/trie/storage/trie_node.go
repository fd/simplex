...
type ReduceFunc func(reductions []ident.SHA, value ident.SHA) ident.SHA

type T interface {
	Store() *storage.S
	Commit() ident.SHA

	Lookup(key []byte) (ident.SHA, bool)
	Insert(key []byte, sha ident.SHA) (old_sha ident.SHA, inserted bool)
	Remove(key []byte) (old_sha ident.SHA, removed bool)
	Reduce(cache T, f ReduceFunc) ident.SHA
...
func Get(s *storage.S, sha ident.SHA) (T, bool) {
	return load_node(s, sha)
}

func load_node(s *storage.S, sha ident.SHA) (*node_t, bool) {
	node := &node_t{
		storage: s,
		sha:     sha,
	}
...
func (s *node_t) Reduce(cache T, f ReduceFunc) ident.SHA {
	return s.reduce(cache, f, []byte{})
}

func (s *node_t) reduce(cache T, f ReduceFunc, prefix []byte) ident.SHA {
	reductions := make([]ident.SHA, len(s.Children))
...
func (s *node_t) Commit() ident.SHA {
	if s.changed != true {
		return s.sha
	}
...
func (s *node_t) Lookup(key []byte) (ident.SHA, bool) {
	node, rem := s.lookup_node_chain(key)

	if len(rem) == 0 {
		if node.Value != ident.ZeroSHA {
...
func (s *node_t) Remove(key []byte) (old_sha ident.SHA, removed bool) {
	node, rem := s.lookup_node_chain(key)
	...
	if node.Set == false {
		return ident.ZeroSHA, true
	}

	old_sha = node.Value

	node.Set = false
	node.Value = ident.ZeroSHA
	node.cleanup()
...
func (s *node_t) Insert(key []byte, sha ident.SHA) (old_sha ident.SHA, inserted bool) {
	node, rem := s.lookup_node_chain(key)
